...
for i in range(len(sections)):
    section_table_list.append([])
    section_image_list.append([])

def split_paragraph_pages(pages, section, page_start_num):
    result = []
    current_section = 0
    txt = ""   
    past_words = ""
    ...
    for page in pages:
        page_dataframe = extract_tables_from_pdf_page(file_path, page_num)
        page_image = extract_images_from_pdf(file_path, page_num)
        if(len(page_dataframe) != 0):
            section_table_list[current_section] += page_dataframe
        if(len(page_image) != 0):
            section_image_list[current_section] += page_image
        page_num += 1
...
def extract_tables_from_pdf_page(file_path, page_num):
...
if(file_type == 'pdf'):
    result = split_paragraph_pages(pages[start_page-1:end_page], sections, start_page)
elif(file_type == 'docx'):
    result = split_paragraph_pages(pages, sections)
result.pop(0)
section_table_list.pop(0)
section_image_list.pop(0)
...
for section, test in matched_tests:
    ...
    table_dict[test].append(section_table_list[i])
    image_dict[test].append(section_image_list[i])
...
for i in range(len(tests)):
    ...
    for j in range(len(tests_match[test])):
        ...
        with open(f"{new_folder_path}/{j}.txt", 'w', encoding='utf-8') as f:
            f.write(section + "\n\n" + result_dict[test][j])
        for text_n in range(len(image_dict[test])):
        #     for image_num in range(len(image_dict[test][text_n])):
        #         image_dict[test][text_n][image_num].save(f"{new_folder_path}/{text_n}_{image_num}_image.jpg")
            for image_num in range(len(image_dict[test][text_n])):
                with open(f"{new_folder_path}/{text_n}_{image_num}_image.jpg", 'w', encoding='utf-8') as f:
                    image_dict[test][text_n][image_num].save(f)
            # with open(f"{new_folder_path}/{image_num}_image.jpg", 'w', encoding='utf-8') as f:
            #     image_dict[test][text_n][].save(f)
        for text_num in range(len(table_dict[test])):
            for table_num in range(len(table_dict[test][text_num])):
                table_dict[test][text_num][table_num].to_csv(f"{new_folder_path}/{text_num}_{table_num}table.csv", index=False)
        


print(section_table_list)